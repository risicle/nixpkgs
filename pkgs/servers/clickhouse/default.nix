...
stdenv.mkDerivation rec {
  ...
  buildInputs = [
    ...
    (cctz.override { inherit stdenv; })
    (poco.override { inherit stdenv; })
    (re2.override { inherit stdenv; })
    (boost.override { inherit stdenv buildPackages; })
    # boost does a charming double-callPackage, so we wrap the callPackage it
    # is handed, giving it one that injects our extra args into calls
    (protobuf.override (oldAttrs: {
